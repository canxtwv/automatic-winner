...
class ZareServiceSpec extends AsyncWordSpec with Matchers with BeforeAndAfterAll {

  /*
  import com.affine.Zare.data.Implicits._
  implicit val system = ActorSystem("zare")
  ...
  lazy val serviceIdentity = ServiceIdentifier("zare")

  lazy val producer = new TypebusTestProducer(serviceIdentity, system)
  //lazy val service = new ZareService(serviceIdentity, producer, system, userDb)

  object service extends ZareService(serviceIdentity, producer, system, userDb){
  ...
  class ZareClient extends Client(serviceIdentity, producer, system){
