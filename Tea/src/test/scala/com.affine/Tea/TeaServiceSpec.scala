...
class TeaServiceSpec extends AsyncWordSpec with Matchers with BeforeAndAfterAll {
  ...
  lazy val serviceIdentity = ServiceIdentifier("tea")

  lazy val producer = new TypebusTestProducer(serviceIdentity, system)
  //lazy val service = new TeaService(serviceIdentity, producer, system, userDb)

  object service extends TeaService(serviceIdentity, producer, system, userDb){
  ...
  class TeaClient extends Client(serviceIdentity, producer, system){
