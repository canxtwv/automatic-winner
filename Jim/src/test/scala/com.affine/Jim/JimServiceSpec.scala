...
class JimServiceSpec extends AsyncWordSpec with Matchers with BeforeAndAfterAll {

  /*
  import com.affine.Jim.data.Implicits._
  implicit val system = ActorSystem("jim")
  ...
  lazy val serviceIdentity = ServiceIdentifier("jim")

  lazy val producer = new TypebusTestProducer(serviceIdentity, system)
  //lazy val service = new JimService(serviceIdentity, producer, system, userDb)

  object service extends JimService(serviceIdentity, producer, system, userDb){
  ...
  class JimClient extends Client(serviceIdentity, producer, system){
