package com.affine.Test.entity
...
class TestEntityDatabase(system: ActorSystem[_])(implicit val ex: ExecutionContext)
  extends TestDatabase with CQRSDatabase[TestState]{
  import akka.util.Timeout
  import scala.concurrent.duration._
  private implicit val askTimeout: Timeout = Timeout(5.seconds)

  val TypeKey = TestEntity.entityTypeKey
  val sharding =  ClusterSharding(system)
  val psEntities: ActorRef[ShardingEnvelope[TestEntity.Command]] =
    sharding.init(Entity(typeKey = TypeKey,
      createBehavior = ctx => TestEntity.behavior(ctx.entityId))
