...
class VegServiceSpec extends AsyncWordSpec with Matchers with BeforeAndAfterAll {

  /*
  import com.affine.Veg.data.Implicits._
  implicit val system = ActorSystem("veg")
  ...
  lazy val serviceIdentity = ServiceIdentifier("veg")

  lazy val producer = new TypebusTestProducer(serviceIdentity, system)
  //lazy val service = new VegService(serviceIdentity, producer, system, userDb)

  object service extends VegService(serviceIdentity, producer, system, userDb){
  ...
  class VegClient extends Client(serviceIdentity, producer, system){
