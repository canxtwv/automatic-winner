...
class TacoServiceSpec extends AsyncWordSpec with Matchers with BeforeAndAfterAll {
  ...
  lazy val serviceIdentity = ServiceIdentifier("taco")

  lazy val producer = new TypebusTestProducer(serviceIdentity, system)
  //lazy val service = new TacoService(serviceIdentity, producer, system, userDb)

  object service extends TacoService(serviceIdentity, producer, system, userDb){
  ...
  class TacoClient extends Client(serviceIdentity, producer, system){
