...
class CarrotServiceSpec extends AsyncWordSpec with Matchers with BeforeAndAfterAll {

  /*
  import com.affine.Carrot.data.Implicits._
  implicit val system = ActorSystem("carrot")
  ...
  //lazy val service = new CarrotService(serviceIdentity, producer, system, userDb)

  object service extends CarrotService(serviceIdentity, producer, system, userDb){
  ...
  class CarrotClient extends Client(serviceIdentity, producer, system){
